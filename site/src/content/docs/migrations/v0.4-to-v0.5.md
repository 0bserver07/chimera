---
title: Migrating from v0.4 to v0.5
description: Step-by-step upgrade guide for chimera v0.4 → v0.5. No breaking API changes for chimera mink users; two new DeprecationWarnings (chimera cc alias, AgentPreset.build) slated for removal in v0.7.0; chimera code REPL default flipped to the assembled CodingAgent stack with --legacy-react as the escape hatch.
---
...
4. **Audit `AgentPreset.build()` call sites.** `grep -r "AgentPreset" .` in your project. Replace per the recipe in the [agent-presets module reference](/chimera/modules/agent-presets).
...
- [Chimera 0.5.0 release notes](/chimera/releases/0.5.0) — the full ship list, live-verification matrix, and quality bar.
- [`AgentPreset` → `CodingAgent` migration recipe](/chimera/modules/agent-presets) — focused recipe for the `AgentPreset.build()` rewrite.
- Per-CLI quickstarts: [mink](/chimera/mink/quickstart) · [otter](/chimera/otter/quickstart) · [ferret](/chimera/ferret/quickstart) · [weasel](/chimera/weasel/quickstart) · [shrew](/chimera/shrew/quickstart).
