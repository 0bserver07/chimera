...
- **⚠️ Python 3.12.8 breaks every Modal call — it is NOT a Modal outage.** Under the venv's CPython **3.12.8**, `asyncio` TLS connects fail *instantly* with `OSError: [Errno 9] Connect call failed ('<ip>', 443)`. The Modal client retries 8× and reports **"Could not connect to the Modal server"**, which reads exactly like an infrastructure outage. It is not: verified 2026-07-24 with status.modal.com fully green while `curl`, `nc`, and system CPython **3.12.9** all reached the identical IP:443 successfully, and only the 3.12.8 venv failed. **Workaround — pin a different interpreter for anything touching Modal:** `uv run --python 3.13 --extra modal-sandbox --extra anthropic modal …` (also applies to `chimera bench-matrix --env modal|swe-modal|e2b|daytona` and `scripts/modal_bench_app.py`). Before ever concluding "Modal is down": check <https://status.modal.com/>, then `curl -s -o /dev/null -w '%{http_code}' https://api.modal.com` (200 = reachable), then retry under `--python 3.13`. This cost hours of misdiagnosis and a dead SWE-bench run.
