...
| L4 | [LiveCodeBench](https://livecodebench.github.io/) | Contest problems (50-problem easy stdin sample) | ~~**Haiku 4.5: 98.0%** (49/50) · **Sonnet 4.6: 98.0%** (49/50)~~ **⊘ RETRACTED 2026-07-28 — do not cite.** `livecodebench` sits in the `RETRACTED` registry in `scripts/render_observatory.py`, so this repo publishes no LiveCodeBench score. The runs are real and their receipts are committed (`data/livecodebench-*-results.json`, both `passed: 49, total: 50`), but the receipts' own `split` field reads `easy-stdin-first-50`: a single-difficulty, single-test-format, first-50 head slice with the LeetCode half of the dataset excluded. That is not a LiveCodeBench score. Corrections: [2026-05-26 fan-out](2026-05-26-claude-codebench-fanout.md), diagnosis in `docs/notes/bench-diagnosis-darklight1.md`. | — | RETRACTED | #12 |
