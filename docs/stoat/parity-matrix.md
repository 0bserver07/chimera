...
| `--output-format` / `--json` | yes | yes | yes | yes | yes | **yes (`--json`)** | yes |
...
| `--shell-mode` | no | no | no | no | no | **yes** | no |
| `--mode <m>` | no | no | no | yes | yes | **yes** | no |
...
| Slash | shared | mink | otter | ferret | **stoat** | badger |
|---|---|---|---|---|---|---|
...
| `/history` | no | no | no | no | **yes** | no |
| `/shell` | no | no | no | no | **yes** | no |
| `/sandbox` | no | no | no | yes | no | no |
| `/approval` | no | no | no | yes | no | no |
| `/parity` | no | no | no | no | no | yes |
| `/rerun` | no | no | no | no | no | yes |
| `/tools` | yes | yes | yes | yes | **(via /tools shared)** | yes |

### Defaults

| Default | Value |
|---|---|
| Default model | `kimi-k2.6` (when `$MOONSHOT_API_KEY` is set), else `claude-sonnet-4-6` |
| Default `--max-steps` | `50` |
| Default `--shell-mode` | `False` (boots in agent mode) |
| Eventlog directory prefix | `~/.chimera/eventlog/stoat-*` |
| Provider chain ordering | Moonshot, Anthropic, OpenAI, OpenRouter, Ollama |
...
- [`../badger/parity-matrix.md`](../badger/parity-matrix.md) — sibling
