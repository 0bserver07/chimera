...
For parallel async runs, instantiate one `Agent` per task and
`asyncio.gather` them. Each `Agent` holds its own provider, its
own context, and its own event log.

```python
async def crawl(prompt: str) -> str:
    agent = Agent(model="claude-sonnet-4-6")
    return (await agent.arun(prompt)).text
