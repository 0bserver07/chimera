...
| [storage-and-experiments](storage-and-experiments.md) | One path registry + `[storage]` config, doctor storage/orphan surface, `chimera gc`, checkpoint hardening, and the `chimera/experiments` toolkit that replaces root-level scratch drivers | 🔵 Design | Plan from the 2026-07-27 root-rot audit (1.3 GB run output + 2 GB orphaned checkpoint); M0–M5 sequenced inside |
