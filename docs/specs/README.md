...
| [coding-agent-harness-integration](coding-agent-harness-integration.md) | Drive the assembled CodingAgent through the eval Harness | 🟢 Shipped | `chimera/eval/coding_agent_adapter.py` (in progress) |
| [agent-benchmark-matrix](agent-benchmark-matrix.md) ([tasks](agent-benchmark-matrix.tasks.md)) | Many-to-many: one `AgentRunner` protocol + registry unifying internal *and* external agents (ACP / CLI-template / native-harness) across the full benchmark registry; `chimera bench matrix` generalizes `bench-compare` to N agents × M benches | 🟡 Partial | **Phase 0 begun:** `chimera bench` registry wired 10→25 (all built adapters, sig-aware loader); `AgentRunner`/`AgentRunResult`/`InProcessRunner` in `chimera/eval/runners/` (tested). Remaining: AgentSpec registry, `bench matrix` CLI, external runners, fidelity table — see tasks file |
| [storage-and-experiments](storage-and-experiments.md) | One path registry + `[storage]` config, doctor storage/orphan surface, `chimera gc`, checkpoint hardening, and the `chimera/experiments` toolkit that replaces root-level scratch drivers | 🟡 Partial | Plan from the 2026-07-27 root-rot audit (1.3 GB run output + 2 GB checkpoint tree from a LIVE writer); M0–M5 sequenced inside. **M1 shipped:** `chimera/config/paths.py` registry (36 stores) + `[storage]` root/retention in the one config chain + every call site migrated (grep-audit clean); guide `docs/guides/storage-and-paths.md`. **M4 shipped:** `chimera/experiments/` (`start`/`resume`, contained run dirs, git-provenance manifest, flushed JSONL ledger, receipt-shaped `result.json`) + `chimera experiments list|show` + guide `docs/guides/experiments.md` + runnable exemplar `scripts/experiments/example_toolkit_run.py`. **M2 shipped:** `chimera doctor --section storage` (per-store size/age/retention + self-reporting orphans, incl. the `.chimera_checkpoints`-position blind spot) and `chimera gc` (dry-run default, opt-in retention, structurally unable to name an unregistered path; the cohort pruner now rides the same engine). **M3 shipped:** one `chimera/config/ignore.py` replacing ELEVEN drifting copies + the checkpoint writer bounded (2,299 MB -> 186.6 MB measured, -91.9%) with symmetric restore exclusion, legacy-dir fallback, and the fake-venv canary. **M5 shipped:** the static cwd-write gate in `tests/test_repo_hygiene.py`. All milestones complete. |
| [modal-amd64-programbench-grading](modal-amd64-programbench-grading.md) | Modal amd64 sandbox for ProgramBench cleanroom grading (unusable under QEMU on arm64 dev machines) | 🔵 Design | Proposal; prior art in `scripts/experiments/scratch_modal_grade.py` (#160, cloud-sandbox track #144) |
| [tui-ux-refinements](tui-ux-refinements.md) | Presentation-quality layer for the shipped TUIs: stream-safe rendering, folding/truncation discipline, customizable status line, themes, rebindable keys — mined from a six-reference corpus | 🔵 Design | 3 phases; phase 1 (correctness) has no config surface. Relates to #170–#172 |
