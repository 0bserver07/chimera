"""Regression tests for ``chimera otter agents`` discovery + presets.

The subcommand surfaces the same project > user > built-in chain that
``--agent <name>`` walks (under otter conventions: ``.opencode/agent``
instead of ``.claude/agents``). Tests cover empty discovery (built-ins
...
also assert the otter built-in preset registry exposes the documented
...
def test_iter_agents_builtins_only(
    ...
    from chimera.otter.agents import iter_agents
    ...
    # Otter advertises this preset set; the registry must expose them.
...
def test_iter_agents_project_dir(tmp_path: Path) -> None:
    """A ``.opencode/agent/local.md`` shows up tagged source=project."""
    _write_agent_md(tmp_path / ".opencode" / "agent", "local", _LOCAL_AGENT)
...
def test_iter_agents_user_dir(
    ...
    """A ``~/.opencode/agent/global.md`` shows up tagged source=user."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: fake_home))  # type: ignore[arg-type]
    _write_agent_md(fake_home / ".opencode" / "agent", "global", _GLOBAL_AGENT)
...
def test_find_agent_project_overrides_user(
    ...
    _write_agent_md(
        fake_home / ".opencode" / "agent", "override", _OVERRIDE_AGENT_USER
    )
    _write_agent_md(
        project_root / ".opencode" / "agent", "override", _OVERRIDE_AGENT_PROJECT
    )

    from chimera.otter.agents import find_agent
...
# ---------------------------------------------------------------------------
# 6. load_otter_agents — returns AgentConfig list with project priority
# ---------------------------------------------------------------------------


def test_load_otter_agents_priority(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    # Project + user define same-named agent; project must win.
    _write_agent_md(
        fake_home / ".opencode" / "agent", "override", _OVERRIDE_AGENT_USER
    )
    _write_agent_md(
    ...
    from chimera.otter.agents import load_otter_agents

    configs = load_otter_agents(project_root=project_root)
...
def test_load_otter_agents_no_dirs(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Without any .opencode/agent dirs, the built-in presets still load."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: fake_home))  # type: ignore[arg-type]
    project_root = tmp_path / "project"
...
def test_cmd_agents_list_prints_table(
    ...
    """``otter agents list`` prints a table with built-in presets."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: fake_home))  # type: ignore[arg-type]
    monkeypatch.chdir(tmp_path)

    from chimera.otter.agents import cmd_agents_list
...
def test_cmd_agents_show_existing(
    ...
    """``otter agents show local`` prints model, tools, and prompt preview."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: fake_home))  # type: ignore[arg-type]
    _write_agent_md(tmp_path / ".opencode" / "agent", "local", _LOCAL_AGENT)
    ...
    from chimera.otter.agents import cmd_agents_show
...
def test_cmd_agents_show_unknown_exits_2(
    ...
    assert ".opencode/agent" in err


def test_cmd_agents_show_missing_name_exits_2(
    capsys: pytest.CaptureFixture[str],
) -> None:
    """``otter agents show`` (no NAME) exits 2 with a usage hint."""
    from chimera.otter.agents import cmd_agents_show
...
def test_format_agents_table_empty() -> None:
    ...
    from chimera.otter.agents import format_agents_table
...
# ---------------------------------------------------------------------------
# 11. CLI dispatch — wired into chimera.otter.cli
# ---------------------------------------------------------------------------


def test_cli_dispatch_agents_list(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """``_dispatch_agents`` wires the list path through cmd_agents_list."""
    import argparse
...
def test_cli_dispatch_agents_show_unknown_exits_2(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    """Unknown agent through cli dispatcher exits 2 with stderr hint."""
    import argparse
