"""Tests for ``chimera.otter.commands`` — custom command ingest.

Covers:

* Project-only scan finds ``.opencode/command/*.md`` files.
* User-only scan finds ``~/.opencode/command/*.md`` files.
...
def test_render_dollar_ten_before_dollar_one() -> None:
    ...
    The longest-first order matters here: a naive replace of ``$1`` would
    turn ``$10`` into ``<arg1>0``.
    """
...
def test_load_project_only(tmp_path: Path) -> None:
    """Project-level ``.opencode/command/*.md`` files load."""
    _write(
        tmp_path / ".opencode" / "command" / "ship.md",
        "---\ndescription: Ship it\n---\nShip the diff.\n",
    )
    _write(
        tmp_path / ".opencode" / "command" / "review.md",
        "Review the changes.\n",
    )
...
def test_load_user_only(tmp_path: Path) -> None:
    ...
    user_dir = tmp_path / "home" / ".opencode" / "command"
...
def test_project_overrides_user(tmp_path: Path) -> None:
    ...
    _write(
        project / ".opencode" / "command" / "ship.md",
...
def test_plural_alias_honored(tmp_path: Path) -> None:
    ...
    project = tmp_path
    _write(
        project / ".opencode" / "command" / "a.md",
        "from singular dir",
    )
    _write(
        project / ".opencode" / "commands" / "b.md",
        "from plural dir",
    )
...
def test_load_handles_missing_dirs(tmp_path: Path) -> None:
    """No ``.opencode/`` dir at all returns an empty mapping (no crash)."""
    cmds = load_custom_commands(tmp_path, user_dirs=())
    assert cmds == {}


def test_load_skips_unreadable_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """An ``OSError`` while reading one file does not abort the whole scan."""
    project = tmp_path
    _write(project / ".opencode" / "command" / "ok.md", "good")
    bad = _write(project / ".opencode" / "command" / "bad.md", "bad")
