...
"""Tests for the harness hook scripts: auto_test, auto_lint, security_scan, verify_done."""
...
class TestHookScripts:
    ...
    def _run_hook(
        ...
        script_path = (
            Path(__file__).resolve().parent.parent.parent / "chimera" / "hooks" / script
        )
        env = os.environ.copy()
        if env_extra:
...
class TestHooksJson:
    ...
    def test_hooks_json_valid(self) -> None:
        ...
        hooks_path = (
            Path(__file__).resolve().parent.parent.parent / "chimera" / "hooks" / "hooks.json"
        )
