"""Tests for ``chimera.shrew.sessions`` (agent S1).

Covers:

* :func:`iter_sessions` walks ``~/.chimera/eventlog/shrew-*`` and skips
  non-shrew sessions.
...
def test_iter_sessions_skips_non_shrew(tmp_path) -> None:
    """``iter_sessions`` only emits ``shrew-*`` directories."""
    _write_session(tmp_path, "shrew-20260430T100000-aaaaaaaa")
    _write_session(tmp_path, "weasel-20260430T100000-bbbbbbbb")
    _write_session(tmp_path, "otter-20260430T100000-cccccccc")
    _write_session(tmp_path, "shrew-20260430T110000-dddddddd")
    records = list(shrew_sessions.iter_sessions(tmp_path))
    ids = [r.session_id for r in records]
    assert ids == [
        "shrew-20260430T110000-dddddddd",
...
def test_iter_sessions_handles_missing_root(tmp_path) -> None:
    ...
    assert list(shrew_sessions.iter_sessions(missing)) == []


def test_iter_sessions_skips_missing_summary(tmp_path) -> None:
    """Sessions without summary.json are skipped."""
    bad = tmp_path / "shrew-20260430T100000-zzzzzzzz"
    bad.mkdir()
    _write_session(tmp_path, "shrew-20260430T110000-cccccccc")
    records = list(shrew_sessions.iter_sessions(tmp_path))
    assert len(records) == 1
    assert records[0].session_id == "shrew-20260430T110000-cccccccc"
...
def test_get_session_loads_summary_and_events(tmp_path) -> None:
    ...
    assert detail.summary["model"] == "qwen3.6-35b-a3b"
...
def test_get_session_unknown_id_raises(tmp_path) -> None:
    with pytest.raises(FileNotFoundError):
        shrew_sessions.get_session("shrew-missing", eventlog_root=tmp_path)
...
def test_get_session_missing_summary_raises(tmp_path) -> None:
    ...
    bad.mkdir()
    with pytest.raises(FileNotFoundError):
        shrew_sessions.get_session(bad.name, eventlog_root=tmp_path)
...
def test_format_session_table_empty() -> None:
    assert (
        shrew_sessions.format_session_table([]) == "(no shrew sessions found)"
    )
...
def test_format_session_table_renders_rows(tmp_path) -> None:
    ...
    records = list(shrew_sessions.iter_sessions(tmp_path))
    out = shrew_sessions.format_session_table(records)
...
def test_format_session_detail_includes_summary_fields(tmp_path) -> None:
    ...
    _write_session(tmp_path, name)
    detail = shrew_sessions.get_session(name, eventlog_root=tmp_path)
    text = shrew_sessions.format_session_detail(detail)
    assert name in text
    assert "qwen3.6-35b-a3b" in text
    assert "do a small thing" in text


def test_session_record_to_dict_round_trips(tmp_path) -> None:
    name = "shrew-20260430T100000-aaaaaaaa"
    _write_session(tmp_path, name)
    record = next(shrew_sessions.iter_sessions(tmp_path))
    payload = record.to_dict()
    assert payload["session_id"] == name
    assert payload["model"] == "qwen3.6-35b-a3b"
    # path is stringified for JSON friendliness.
...
def test_cmd_sessions_list_text(tmp_path) -> None:
    _write_session(tmp_path, "shrew-20260430T100000-aaaaaaaa")
    buf = io.StringIO()
    rc = shrew_sessions.cmd_sessions_list(eventlog_root=tmp_path, out=buf)
    assert rc == 0
...
def test_cmd_sessions_list_json(tmp_path) -> None:
    _write_session(tmp_path, "shrew-20260430T100000-aaaaaaaa")
    buf = io.StringIO()
    rc = shrew_sessions.cmd_sessions_list(
        eventlog_root=tmp_path, json_output=True, out=buf,
    )
    assert rc == 0
    payload = json.loads(buf.getvalue())
    assert isinstance(payload, list)
    assert payload[0]["session_id"] == "shrew-20260430T100000-aaaaaaaa"


def test_cmd_sessions_show_missing_id_is_usage_error() -> None:
    err = io.StringIO()
    rc = shrew_sessions.cmd_sessions_show(None, err=err)
    assert rc == 2
...
def test_cmd_sessions_show_unknown_id(tmp_path) -> None:
    err = io.StringIO()
    rc = shrew_sessions.cmd_sessions_show(
...
def test_dispatch_sessions_list_default(monkeypatch) -> None:
    ...
    monkeypatch.setattr(shrew_sessions, "cmd_sessions_list", _fake_list)
...
def test_dispatch_sessions_show_routes_target(monkeypatch) -> None:
    ...
    monkeypatch.setattr(shrew_sessions, "cmd_sessions_show", _fake_show)
    args = argparse.Namespace(
        sub_action="show",
        sub_target="shrew-abc",
        json_output=True,
    )
    rc = shrew_sessions.dispatch_sessions(args)
    assert rc == 0
...
def test_default_eventlog_root_is_under_home() -> None:
    root = shrew_sessions.default_eventlog_root()
