...
``/session/<id>/cancel``          POST   Cooperatively cancel an in-flight
                                         ...
                                         ``204 No Content``.
``/session/<id>/rotate-token``    POST   Rotate the per-session bearer token.
                                         Master-token-only (admin). Returns
                                         ``{"session_token": "..."}``. The old
...
Two token tiers when ``--auth-token`` is configured:

* **Master token** (``--auth-token``) — admin level. Authorizes every route
  including admin operations like ``POST /session/<id>/rotate-token`` and
  the ``GET /sessions`` listing.
* **Per-session tokens** — auto-generated by ``secrets.token_urlsafe(32)``
  on session creation, returned in the ``POST /session`` response under
  ``session_token``. A session token authorizes only requests against its
  own ``/session/<id>/...`` subtree; it cannot create new sessions, rotate
  tokens, or touch other sessions. Stored in-memory only; not persisted
...
class OtterSessionState:
    ...
    Attributes:
        ...
        session_token: Per-session bearer token issued at create time by
            :meth:`OtterSessionManager.create`. Authorizes requests
            against this session's ``/session/<id>/...`` subtree only,
            on top of (or in place of) the master ``--auth-token``.
            Generated via :func:`secrets.token_urlsafe` (32 bytes of
            entropy); rotated by :meth:`OtterServer.rotate_session_token`.
            ``None`` is permitted only as a transient default for
            backwards compatibility with hand-built
            :class:`OtterSessionState` instances in tests; the real
            manager always populates it.
    """
...
class OtterSessionManager:
    ...
    def create(self, *, working_dir: str = "") -> OtterSessionState:
        ...
        state = OtterSessionState(
            ...
            created_at=now,
            last_touched=now,
            # B8 (wave-11): per-session bearer token. ``secrets.token_urlsafe``
            # gives a cryptographically random URL-safe string; 32 bytes of
            # entropy keeps tokens distinct from the master ``--auth-token``
            # and indistinguishable from random across the lifetime of the
            # process. The token lives only in memory — server restart
            # invalidates every session token (and every session, anyway).
...
class OtterServer:
    ...
    def rotate_session_token(self, session_id: str) -> str | None:
        """Replace *session_id*'s ``session_token`` with a fresh one.

        Returns the new token on success, ``None`` if the session is
        unknown. The previous token is invalidated immediately — any
        request still using it will be rejected by the auth check on the
        next handler dispatch. Touches ``last_touched`` so a rotation
        keeps the session alive against TTL eviction.

        Admin-only: the HTTP handler enforces master-token-only auth on
        ``POST /session/<id>/rotate-token`` before calling this method.
        Library callers (Python embedders) carry no such gate; this
        method just does the swap.
        """
        # Use the manager's get() so ``last_touched`` is bumped — a
        # rotation is observable activity.
    ...
    def _build_handler_class(self) -> type[BaseHTTPRequestHandler]:
        ...
        class _Handler(BaseHTTPRequestHandler):
            ...
            def _check_auth(self) -> bool:
                """Verify the request bearer token (master OR per-session).

                B8 auth decision tree (when ``outer._auth_token`` is set):

                * ``GET /healthz`` — always open.
                * Path under ``/session/<id>/...`` (and not the admin-only
                  ``rotate-token`` route): authorized if the bearer
                  matches EITHER the master token OR ``state.session_token``
                  for the *same* session id. A session A token presented
                  for session B fails — tokens are scoped to their issuing
                  session.
                * Every other route — including ``POST /session`` (create),
                  ``GET /sessions``, ``GET /session`` (list), ``POST
                  /session/<id>/rotate-token``, and ``POST /tool/approve``
                  — requires the master ``--auth-token``. Session tokens
                  are intentionally insufficient for admin / cross-session
                  operations.

                Returns ``True`` when authorized; otherwise writes a 401
                ...
                master_expected = f"Bearer {outer._auth_token}"
                if got == master_expected:
                    return True
                # Per-session token path: every endpoint under
                # ``/session/<id>/`` (and the bare ``/session/<id>``
                # snapshot route) accepts the session's own token. The
                # admin-only sub-routes (currently ``rotate-token``)
                # gate themselves with a second check inside their
                # handler so a session-token caller sees a 403, not a
                # generic 401 — the request is authenticated, just not
                # privileged.
                path = self.path.split("?", 1)[0]
                if path.startswith("/session/"):
                    parts = path.split("/")
                    # parts == ["", "session", "<id>", "<sub?>"]
                    if len(parts) >= 3 and parts[2]:
                        session_id = parts[2]
                        # Don't ``touch`` here — auth check is part of
                        # the same request that will hit the handler,
                        # which already does its own ``get_session``
                        # and bumps ``last_touched``.
                        state = outer._session_manager.get(
                                session_id, touch=False
                        )
                        if (
                            state is not None
                            and state.session_token is not None
                            and got == f"Bearer {state.session_token}"
                        ):
                            return True
            ...
            def do_POST(self) -> None:  # noqa: N802 - stdlib API
                ...
                if path.startswith("/session/"):
                    ...
                    if len(parts) == 4 and parts[3] == "rotate-token":
                        return self._handle_session_rotate_token(parts[2])
            ...
            def _handle_session_create(self) -> None:
                ...
                # B8: include the per-session bearer token in the create
                # response so a multi-tenant client can hand the token
                # off to a less-privileged caller without leaking the
                # master ``--auth-token``. The manager always issues a
                # token at create time; the server-wide ``--auth-token``
                # flag controls whether either token is *enforced*, but
                # the per-session token is always returned so
                # rotate-token can invalidate a forwarded handle even on
                # an open server.
                if state.session_token is not None:
                    payload["session_token"] = state.session_token
            ...
            def _handle_session_rotate_token(self, session_id: str) -> None:
                """Rotate *session_id*'s ``session_token``. Master-only.

                B8 admin gate: ``_check_auth`` already accepts session
                tokens for most ``/session/<id>/...`` routes, so we
                re-check the master token explicitly here. Returning 403
                (rather than 401) signals "the request is authenticated
                but the principal is not allowed to perform this
                operation" — exactly what a session-token caller hits
                when it tries to rotate.
                """
                # Drain any body — the route ignores the body, but
                # urllib clients block until it's consumed.
                length = int(self.headers.get("Content-Length", "0") or "0")
                if length > 0:
                    self.rfile.read(length)
                if outer._auth_token is not None:
                    got = self.headers.get("Authorization", "")
                    master_expected = f"Bearer {outer._auth_token}"
                    if got != master_expected:
                        # ``_check_auth`` already let the request
                        # through (possibly via a session token); deny
                        # the privileged sub-operation with 403.
                        self._send_json(403, {"error": "admin_only"})
                        return
                new_token = outer.rotate_session_token(session_id)
                if new_token is None:
                    self._send_json(404, {"error": "session_not_found"})
                    return
                self._send_json(200, {"session_token": new_token})
